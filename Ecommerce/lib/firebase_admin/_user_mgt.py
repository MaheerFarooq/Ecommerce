...
class _Validator(object):
    ...
    def validate_uid(cls, uid):
        if not isinstance(uid, lib.six.string_types) or not uid or len(uid) > 128:
            raise ValueError(
    ...
    def validate_email(cls, email):
        if not isinstance(email, lib.six.string_types) or not email:
            raise ValueError(
    ...
    def validate_phone(cls, phone):
        ...
        """
        if not isinstance(phone, lib.six.string_types) or not phone:
    ...
    def validate_password(cls, password):
        if not isinstance(password, lib.six.string_types) or len(password) < 6:
            raise ValueError(
    ...
    def validate_display_name(cls, display_name):
        if not isinstance(display_name, lib.six.string_types) or not display_name:
            raise ValueError(
    ...
    def validate_photo_url(cls, photo_url):
        if not isinstance(photo_url, lib.six.string_types) or not photo_url:
            raise ValueError(
...
class UserManager(object):
    ...
    def __init__(self, app):
        ...
        version_header = 'Python/Admin/{0}'.format(lib.firebase_admin.__version__)
